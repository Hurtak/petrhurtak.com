...
async function prepareDirs(done) {
  ...
  await Promise.all([fs.mkdir(paths.distDrafts), fs.mkdir(paths.distStatic)]);
  await Promise.all([
...
async function compileStyles(done, productionBuild) {
  ...
  await Promise.all([
    fs.writeFile(
      path.join(paths.distStyles, `styles.${hashCss}.css`),
      result.css
    ),
...
async function compileScripts(done, productionBuild) {
  ...
  const baseTemplate = fs.readFileSync(
    path.join(paths.templates, "/extends/base.njk"),
    "utf8"
  );
  ...
  await fs.writeFile(
    path.join(paths.distScripts, `scripts.${hashJs}.js`),
    code
  );
...
function gatherArticlesData(done) {
  let allArticles = articles.getArticles(
    paths.articles,
  ...
  let publishedArticles = lodash.filter(
    allArticles,
    article => article.metadata.published === true
  );
  publishedArticles = lodash.filter(
    publishedArticles,
    article => article.metadata.dateLastUpdate <= new Date()
  );
...
async function compilePages(done, productionBuild) {
  const pages = [
    ...
    { from: "robots.txt.njk", to: "robots.txt", data: null },
    {
      ...
      to: "index.html",
      data: {
        articles: lodash.slice(
          articlesData.published,
          0,
          config.articles.perPage
        )
      }
    },
    {
    ...
    },
    {
      ...
      data: {
        lastUpdate:
          articlesData.published.length > 0
...
async function compileArticles(done, productionBuild) {
  for (const article of articlesData.all) {
    ...
    if (productionBuild) {
      ...
      const videosPath = path.join(article.fs.path, paths.articleVideos);
...
async function testCoverage(done) {
  try {
    await execa.shell(
      'nyc --all --include="src" --exclude="src/test" ava src/test/**/*.js'
    );
  } catch (e) {
    console.log(e.stderr);
  }
...
gulp.task(
  "site:compile",
  gulp.series(
    gulp.parallel("site:prepare-dirs", "site:gather-articles-data"),
    gulp.parallel(
      "site:styles",
      "site:scripts",
      "site:images",
...
gulp.task("watch:test", () =>
  gulp.watch(["./src/**/*.js", "./articles/**/*"], gulp.series("test:all"))
);
...
gulp.task(
  ...
  gulp.series(
    "site:compile",
    "browser-sync:server",
    gulp.parallel("test:all", "watch:articles", "watch:styles", "watch:test")
  )
);
