...
function enhanceSnippetLinks(htmlString, snippets, nunjucks) {
  ...
  $('a[href^="./snippets/"]').each((_, element) => {
    ...
    const snippet = snippets.find(
      snippet => snippet.metadata.name === snippetName
    );
    if (!snippet) return;
...
function removeIndentation(str) {
  ...
  lines = lodash.map(lines, line =>
    lodash.replace(line, indentationToRemove, "")
  );
...
function parseSnippet(wholeHtml) {
  ...
  const nullIfEmptyString = str =>
    typeof str === "string" && str.trim() === "" ? null : str;
