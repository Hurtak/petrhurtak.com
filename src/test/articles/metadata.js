...
const articlesData = articles.getArticles(paths.articles, paths.articlesDrafts, nunjucks)

test('Required items exist and have correct type', t => {
  const required = [
    ['title', title => lodash.isString(title) && title.length >= 2],
    ['description', description => lodash.isString(description) && description.length >= 2],
    ['url', url => lodash.isString(url) && url.length >= 2],
    ['datePublication', date => lodash.isDate(date) && date >= Date.UTC(2015, 0) && date < Date.UTC(2020, 0)],
    ['dateLastUpdate', date => lodash.isDate(date) && date >= Date.UTC(2015, 0) && date < Date.UTC(2020, 0)],
