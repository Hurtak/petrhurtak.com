...
test("datetimeAttribute", t => {
  ...
  const dateToDateTimeAttribute = givenDate => {
    ...
    return `${d.year}-${addLeadingZero(d.month)}-${addLeadingZero(
      d.day
    )}T${addLeadingZero(d.hours)}:${addLeadingZero(d.minutes)}:${addLeadingZero(
      d.seconds
    )}`;
  };
...
test("gmt", t => {
  ...
  const dateToGmt = givenDate => {
    ...
    return `${dayNames[d.dayOfWeek]}, ${addLeadingZero(d.day)} ${
      monthNames[d.month - 1]
    } ${d.year} ${addLeadingZero(d.hours)}:${addLeadingZero(
      d.minutes
    )}:${addLeadingZero(d.seconds)} GMT`;
  };
